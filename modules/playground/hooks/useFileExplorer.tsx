...
// zustand's `create<T>()` currying occasionally fails to infer the store
// shape against a large hand-written interface like this one - suppressing
// here rather than loosening FileExplorerState itself.
// @ts-ignore
export const useFileExplorer = create<FileExplorerState>((set, get) => ({
  ...
  handleAddFile:async(newFile , parentPath , writeFileSync , instance , saveTemplateData)=>{
    ...
    try {
      ...
      // Keep the running WebContainer filesystem in sync - otherwise the
      // dev server keeps serving a file the explorer no longer shows.
      if (instance?.fs) {
        const filePath = parentPath
        ...
        try {
          ...
          console.error("Failed to remove file from WebContainer fs:", fsError);
        }
      }
    ...
    try {
      ...
      // Keep the running WebContainer filesystem in sync.
      if (instance?.fs) {
        const folderPath = parentPath
        ...
        try {
          ...
          console.error("Failed to remove folder from WebContainer fs:", fsError);
        }
      }
      ...
      if (fileIndex !== -1) {
        ...
        if (instance?.fs) {
          ...
          try {
            ...
            console.error("Failed to rename file in WebContainer fs:", fsError);
          }
        }
      ...
      if (folderIndex !== -1) {
        ...
        if (instance?.fs) {
          ...
          try {
            ...
            console.error("Failed to rename folder in WebContainer fs:", fsError);
          }
        }
