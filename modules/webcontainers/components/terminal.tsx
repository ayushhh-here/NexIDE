...
TerminalComponent = forwardRef<TerminalRef, TerminalProps>(({ 
  ...
  // "cd" is a shell builtin, not a real binary in the container's
  // filesystem - spawning it as a process always fails. Track the working
  // directory ourselves and pass it as `cwd` to every spawned command.
  const currentDir = useRef<string>(".");

  const resolvePath = useCallback((base: string, target: string): string => {
    if (!target || target === "~") return ".";
    // Absolute path -> start from the container root; relative -> walk from base.
  ...
  // Runs a single command (no "&&"/"|") against the WebContainer and
  // returns its exit code, so callers can chain commands together.
  const runSingleCommand = useCallback(async (command: string): Promise<number> => {
    if (!webContainerInstance || !term.current) return 1;
    ...
    if (trimmed === "pwd") {
      term.current.writeln(`/${currentDir.current === "." ? "" : currentDir.current}`);
      return 0;
    }
    ...
    if (cmd === "cd") {
      const target = args[0] || ".";
      const newPath = resolvePath(currentDir.current, target);
      try {
        await webContainerInstance.fs.readdir(newPath === "." ? "." : newPath);
  ...
  }, [webContainerInstance, resolvePath]);
  ...
  const executeCommand = useCallback(async (command: string) => {
    ...
    if (command.includes("|")) {
      term.current.writeln("\r\nPipes ( | ) aren't supported in this terminal yet - run commands separately.");
      writePrompt();
      return;
    }

    // Basic "&&" chaining: run each command in order, stop at first failure.
    const chained = command.split("&&");

    for (const step of chained) {
      const exitCode = await runSingleCommand(step);
      if (exitCode !== 0 && chained.length > 1) break;
    }

    writePrompt();
  }, [webContainerInstance, runSingleCommand, writePrompt]);
    ...
    switch (data) {
      ...
      case '\u001b[A': // Up arrow
        if (commandHistory.current.length > 0) {
          ...
          term.current.write(`\r${promptLabel()} $ ` + ' '.repeat(currentLine.current.length) + `\r${promptLabel()} $ `);
  ...
  }, [executeCommand, writePrompt, promptLabel]);
